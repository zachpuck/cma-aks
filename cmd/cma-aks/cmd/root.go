...
import (
	...
	"github.com/samsung-cnct/cma-aks/pkg/apiserver"
...
func runWebServer() {
	logger := util.GetModuleLogger("cmd.cmaaks", loggo.INFO)
...
func init() {
	...
	viper.SetEnvPrefix("cmaaks")
	replacer := strings.NewReplacer("-", "_")
	viper.SetEnvKeyReplacer(replacer)

	// using standard library "flag" package
	RootCmd.Flags().Int("port", 9030, "Port to listen on")
